...
namespace {
	...
	int DoIt( int argc, char * argv[], T ) {
		...
		typename ReaderType::Pointer reader1;
		typename ReaderType::Pointer reader2;
		typename RegionFilterType::Pointer regionFilter1;
		typename RegionFilterType::Pointer regionFilter2;

		typename Interpolator::Pointer interp;
		typename ResampleType::Pointer resample;
		...
		// ROI
		itk::ImageRegion<3> region1;
		itk::ImageRegion<3> region2;
		...
		regionFilter1 = RegionFilterType::New();
		itk::PluginFilterWatcher watchRegion1( regionFilter1, "Region Filter 1", CLPProcessInformation );
		regionFilter1->SetInput( reader1->GetOutput() );
		regionFilter1->SetRegionOfInterest( region1 );
		regionFilter1->Update();

		regionFilter2 = RegionFilterType::New();
		itk::PluginFilterWatcher watchRegion2( regionFilter2, "Region Filter 2", CLPProcessInformation );
		...
		itk::PluginFilterWatcher watchFilter( filter, "Absolute Difference", CLPProcessInformation );
		filter->SetInput1( diffInput1 );
		filter->SetInput2( diffInput2 );
